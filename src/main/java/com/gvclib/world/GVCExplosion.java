        ...
        if (this.damagesTerrain)
        {
            for (BlockPos blockpos : this.affectedBlockPositions)
            {
                ...
                if (iblockstate.getMaterial() != Material.AIR)
                {
                    ...
                    {
                    	if(block == Blocks.SAND ||block == Blocks.SANDSTONE || block == Blocks.DIRT || block == Blocks.GRASS || block == Blocks.NETHERRACK  || block == Blocks.SNOW_LAYER) {
                    		//block.dropBlockAsItemWithChance(this.world, blockpos, this.world.getBlockState(blockpos), 1.0F / this.size, 0);
                    		block.dropBlockAsItemWithChance(this.world, blockpos, this.world.getBlockState(blockpos), 0, 0);
                    	}else {
                    		block.dropBlockAsItemWithChance(this.world, blockpos, this.world.getBlockState(blockpos), 1.0F * (float)mod_GVCLib.cfg_explotion_drop, 0);
                    	}
        ...
        if (this.causesFire)
        {
        	 if (this.damagesTerrain)
             {
            	 ...
            	 for (BlockPos blockpos1 : this.affectedBlockPositions)
                 {
                     if (this.world.getBlockState(blockpos1).getMaterial() == Material.AIR && this.world.getBlockState(blockpos1.down()).isFullBlock() && this.random.nextInt(3) == 0)
                     {
                         this.world.setBlockState(blockpos1, mod_GVCLib.b_fire.getDefaultState());
                     }
                 }
             }
        }
